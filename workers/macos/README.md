...
```bash
conda run -n DV python workers/macos/worker.py --mode replay --once
```

`replay` 모드에서는 `workers/macos/goodnotes_controller.py`를 호출한다. `config.json`의 `execute_goodnotes_controller`가 `false`이면 dry-run으로 실행한 뒤 stale pasteboard 업로드를 막기 위해 job을 실패 처리한다. 실제 입력, 올가미 복사, binary 추출을 수행하려면 `true`로 바꾸고 GoodNotes 창, 펜 도구, 화면 좌표를 먼저 고정해야 한다. 기존 `execute_goodnotes_writer` 값도 호환용으로 계속 인식한다.
...
```bash
conda run -n DV python workers/macos/preflight.py --check-swift
```
...
```bash
conda run -n DV python workers/macos/preflight.py \
  --strokes backend/storage/jobs/<job_id>/crop_stroke_composite_strokes.json \
  --check-swift
```
...
conda run -n DV python goodnotes_writer.py \
  --strokes workers/macos/samples/single_stroke.json \
...
```bash
conda run -n DV python workers/macos/calibrate_input.py \
...
```bash
conda run -n DV python workers/macos/goodnotes_controller.py \
  --strokes workers/macos/samples/crop_stroke_composite_strokes.json \
  --target_rect 320,180,980,720 \
  --sample_step 10 \
