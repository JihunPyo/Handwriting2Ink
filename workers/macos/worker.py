...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="workers/macos/config.json", help="워커 설정 JSON 경로")
...
def job_work_dir(config: dict[str, Any], job_id: str) -> Path:
    root = Path(config.get("jobs_dir", "workers/macos/jobs"))
...
def run_goodnotes_controller(config: dict[str, Any], strokes_path: Path) -> bool:
    ...
    command = [
        sys.executable,
        str(WORKER_ROOT / "goodnotes_controller.py"),
        "--config",
        config.get("__config_path", str(WORKER_ROOT / "config.json")),
        "--strokes",
        str(strokes_path),
    ]
...
def dump_goodnotes_clipboard(output_path: Path) -> None:
    script_path = WORKER_ROOT / "dump_goodnotes_clipboard.swift"
    env = os.environ.copy()
