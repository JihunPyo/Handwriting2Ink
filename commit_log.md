...
## 2026-09-01 포트폴리오 README 초안 작성

- 루트 `README.md`에 프로젝트 소개, 문제의식, 결과 화면, 시스템 아키텍처, 핵심 기술, Quick Start, 상세 문서 링크, 한계와 팀 구성을 정리했다.
- H2I 발표자료에서 전처리 비교, stroke 파이프라인, 시스템 아키텍처, iPad·Mac GUI 화면을 추출해 `docs/assets/`에 추가했다.
- 최종 데모 영상 URL, Goodnotes 최종 Before/After 화면, 팀원별 담당 역할은 사용자가 보완할 항목으로 표시했다.
