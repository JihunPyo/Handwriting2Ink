...
```bash
conda run -n DV python -m uvicorn server.app.main:app --reload --host 0.0.0.0 --port 8000
```
...
```bash
H2I_STROKE_MODE=pipeline conda run -n DV python -m uvicorn server.app.main:app --reload --host 0.0.0.0 --port 8000
```
