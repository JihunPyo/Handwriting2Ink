...
본 문서는 `0531개발계획서.md`의 전체 구조를 먼저 연결하기 위한 개발용 뼈대를 설명한다.
...
```bash
conda run -n DV python -m pip install -r server/requirements.txt
conda run -n DV python -m uvicorn server.app.main:app --host 0.0.0.0 --port 8000
```
...
기본 설정은 `mac_worker/config.json`에 있다.
...
`GoodNotesRestoreApp/GoodNotesRestoreApp` 아래 Swift 파일은 iPad 앱 target에 추가해서 사용할 수 있는 뼈대이다.
