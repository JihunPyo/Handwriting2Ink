...
Handwriting2Ink는 종이에 작성한 필기를 촬영해 업로드하면, 필기 위치와 형태를 stroke 좌표로 복원하고 Goodnotes에서 붙여넣을 수 있는 필기 객체로 변환하는 서비스이다.

단순히 이미지를 문서에 삽입하는 데서 끝나지 않는다. OCR 기반 레이아웃 분석과 skeleton 기반 stroke 추출을 거쳐, 사용자가 Goodnotes에서 선택·이동·편집할 수 있는 ink 데이터로 다시 만든다.
...
종이 필기는 자유롭게 메모하고 구조를 표현하기 좋지만 검색·분류·재사용이 어렵다. 반대로 전자 필기 앱은 관리가 편리하지만, 기존 종이 필기를 옮길 때는 이미지나 PDF처럼 평면화된 결과만 남는 경우가 많다.

기존 방식에는 다음과 같은 한계가 있다.

- OCR은 글자를 텍스트로 바꾸지만 필기체의 형태와 공간 배치를 보존하기 어렵다.
- 스캔 이미지는 Goodnotes에서 개별 획로 선택하거나 수정할 수 없다.
- 사람이 필기를 다시 따라 그리는 방식은 문서가 많을수록 반복 비용이 커진다.

Handwriting2Ink는 종이 필기의 자유도와 전자 필기 앱의 편집 가능성을 연결하는 것을 목표로 한다.
